 ...
 * @brief i2c port,easy component migration。Highly recommended to use function pointers in your library
 * @version 0.1
 * @date 2023-07-12
 * 
...
uint32_t twi_read(uint8_t dev_addr, uint8_t reg_addr, uint8_t *data, uint8_t len)   //! 类型错误：uint16_t
{
    ...
    if (reg_addr != 0xFF) {
        i2c_master_write_byte(cmd, reg_addr, ACK_CHECK_EN); //Single Read and Write
    }
    i2c_master_stop(cmd);
